...
def submit_user_details(request):
    if request.method == 'POST':
        ...
        if serializer.is_valid():
            ...
            user, created = UserDetail.objects.get_or_create(email=email, defaults=serializer.validated_data)
            
            if created:
                return Response(serializer.data, status=status.HTTP_201_CREATED)  # User created
            else:
                return Response({"detail": "User already exists"}, status=status.HTTP_200_OK)  # User already exists
